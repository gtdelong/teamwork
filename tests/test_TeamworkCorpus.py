import pytest
from teamwork import teamwork as tw
from utils import utils as u
import pandas as pd


def test_team_size():
    # Arrange
    test_df = pd.DataFrame.from_dict(data_hf, orient="index")
    for col in ['date', 'arrive_date']:
        test_df[col] = pd.to_datetime(test_df[col])

    # Act
    corpus = tw.TeamworkCorpus(test_df)
    actual_teamsize = len(corpus.team_experience_dict[test_visit_id]['team'])

    # Assert
    expected_teamsize = 4
    assert expected_teamsize == actual_teamsize
    
def test_edge_size():
    # Arrange
    test_df = pd.DataFrame.from_dict(data_hf, orient="index")
    for col in ['date', 'arrive_date']:
        test_df[col] = pd.to_datetime(test_df[col])

    # Act
    corpus = tw.TeamworkCorpus(test_df)
    actual_edgesize = len(corpus.visit_id_to_edges_dict[test_visit_id])

    # Assert
    expected_edgesize = 6
    expected_edgesize == actual_edgesize

test_visit_id = 6

data = {
    0: {
        "age": 75,
        "arrive_date": "2019-01-01 00:00:00",
        "date": "2019-01-01 19:15:00",
        "discharge_date": "2019-01-01",
        "disposition": 1,
        "dr": "Brad Palmer",
        "id": 0,
        "patient": "patient1",
    },
    1: {
        "age": 68,
        "arrive_date": "2019-01-24 00:00:00",
        "date": "2019-01-24 10:19:00",
        "discharge_date": "2019-01-24",
        "disposition": 0,
        "dr": "Albert Romero",
        "id": 1,
        "patient": "patient2",
    },
    2: {
        "age": 68,
        "arrive_date": "2019-01-24 00:00:00",
        "date": "2019-01-24 17:09:00",
        "discharge_date": "2019-01-24",
        "disposition": 0,
        "dr": "Margie Meyer",
        "id": 1,
        "patient": "patient2",
    },
    3: {
        "age": 68,
        "arrive_date": "2019-01-24 00:00:00",
        "date": "2019-01-24 16:48:00",
        "discharge_date": "2019-01-24",
        "disposition": 0,
        "dr": "Evan Frazier",
        "id": 1,
        "patient": "patient2",
    },
    8: {
        "age": 71,
        "arrive_date": "2019-02-14 00:00:00",
        "date": "2019-02-14 20:58:00",
        "discharge_date": "2019-02-14",
        "disposition": 0,
        "dr": "Albert Romero",
        "id": 2,
        "patient": "patient3",
    },
    9: {
        "age": 71,
        "arrive_date": "2019-02-14 00:00:00",
        "date": "2019-02-14 15:54:00",
        "discharge_date": "2019-02-14",
        "disposition": 0,
        "dr": "Margie Meyer",
        "id": 2,
        "patient": "patient3",
    },
    12: {
        "age": 66,
        "arrive_date": "2019-04-15 19:15:00",
        "date": "2019-04-16 01:29:00",
        "discharge_date": "2019-04-15",
        "disposition": 0,
        "dr": "Albert Romero",
        "id": 6,
        "patient": "patient4",
    },
    13: {
        "age": 66,
        "arrive_date": "2019-04-15 19:15:00",
        "date": "2019-04-15 20:19:00",
        "discharge_date": "2019-04-15",
        "disposition": 0,
        "dr": "Margie Meyer",
        "id": 6,
        "patient": "patient4",
    },
    14: {
        "age": 66,
        "arrive_date": "2019-04-15 19:15:00",
        "date": "2019-04-16 04:43:00",
        "discharge_date": "2019-04-15",
        "disposition": 0,
        "dr": "Evan Frazier",
        "id": 6,
        "patient": "patient4",
    },
    15: {
        "age": 66,
        "arrive_date": "2019-04-15 19:15:00",
        "date": "2019-04-18 21:23:00",
        "discharge_date": "2019-04-15",
        "disposition": 0,
        "dr": "Myrtle George",
        "id": 6,
        "patient": "patient4",
    },
    16: {
        "age": 66,
        "arrive_date": "2019-04-15 19:15:00",
        "date": "2019-04-16 07:00:00",
        "discharge_date": "2019-04-15",
        "disposition": 0,
        "dr": "Victoria Washington",
        "id": 6,
        "patient": "patient4",
    },
    22: {
        "age": 69,
        "arrive_date": "2019-08-15 00:00:00",
        "date": "2019-08-15 17:07:00",
        "discharge_date": "2019-08-15",
        "disposition": 1,
        "dr": "Neil Mitchell",
        "id": 7,
        "patient": "patient5",
    },
}

data_hf = {   0: {   'age': 75,
           'arrive_date': '2019-01-01 00:00:00',
           'date': '2019-01-01 19:15:00',
           'discharge_date': '2019-01-01',
           'disposition': 1,
           'dr': 'Brad Palmer',
           'hf': True,
           'id': 0,
           'patient': 'patient1'},
    1: {   'age': 68,
           'arrive_date': '2019-01-24 00:00:00',
           'date': '2019-01-24 10:19:00',
           'discharge_date': '2019-01-24',
           'disposition': 0,
           'dr': 'Albert Romero',
           'hf': False,
           'id': 1,
           'patient': 'patient2'},
    2: {   'age': 68,
           'arrive_date': '2019-01-24 00:00:00',
           'date': '2019-01-24 17:09:00',
           'discharge_date': '2019-01-24',
           'disposition': 0,
           'dr': 'Margie Meyer',
           'hf': False,
           'id': 1,
           'patient': 'patient2'},
    3: {   'age': 68,
           'arrive_date': '2019-01-24 00:00:00',
           'date': '2019-01-24 16:48:00',
           'discharge_date': '2019-01-24',
           'disposition': 0,
           'dr': 'Evan Frazier',
           'hf': False,
           'id': 1,
           'patient': 'patient2'},
    8: {   'age': 71,
           'arrive_date': '2019-02-14 00:00:00',
           'date': '2019-02-14 20:58:00',
           'discharge_date': '2019-02-14',
           'disposition': 0,
           'dr': 'Albert Romero',
           'hf': True,
           'id': 2,
           'patient': 'patient3'},
    9: {   'age': 71,
           'arrive_date': '2019-02-14 00:00:00',
           'date': '2019-02-14 15:54:00',
           'discharge_date': '2019-02-14',
           'disposition': 0,
           'dr': 'Margie Meyer',
           'hf': True,
           'id': 2,
           'patient': 'patient3'},
    12: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-16 01:29:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Albert Romero',
            'hf': True,
            'id': 6,
            'patient': 'patient4'},
    13: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-15 20:19:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Margie Meyer',
            'hf': True,
            'id': 6,
            'patient': 'patient4'},
    14: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-16 04:43:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Evan Frazier',
            'hf': True,
            'id': 6,
            'patient': 'patient4'},
    15: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-18 21:23:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Myrtle George',
            'hf': True,
            'id': 6,
            'patient': 'patient4'},
    16: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-16 07:00:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Victoria Washington',
            'hf': True,
            'id': 6,
            'patient': 'patient4'},
    22: {   'age': 69,
            'arrive_date': '2019-08-15 00:00:00',
            'date': '2019-08-15 17:07:00',
            'discharge_date': '2019-08-15',
            'disposition': 1,
            'dr': 'Neil Mitchell',
            'hf': False,
            'id': 7,
            'patient': 'patient5'}}

data_sm_result = {   15: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-18 21:23:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Myrtle George',
            'hf': True,
            'id': 6,
            'is_in_team': False,
            'norm_admission_date': '2019-04-15',
            'norm_note_date': '2019-04-18',
            'patient': 'patient4'},
    16: {   'age': 66,
            'arrive_date': '2019-04-15 19:15:00',
            'date': '2019-04-16 07:00:00',
            'discharge_date': '2019-04-15',
            'disposition': 0,
            'dr': 'Victoria Washington',
            'hf': True,
            'id': 6,
            'is_in_team': True,
            'norm_admission_date': '2019-04-15',
            'norm_note_date': '2019-04-16',
            'patient': 'patient4'},
    22: {   'age': 69,
            'arrive_date': '2019-08-15 00:00:00',
            'date': '2019-08-15 17:07:00',
            'discharge_date': '2019-08-15',
            'disposition': 1,
            'dr': 'Neil Mitchell',
            'hf': False,
            'id': 7,
            'is_in_team': False,
            'norm_admission_date': '2019-08-15',
            'norm_note_date': '2019-08-15',
            'patient': 'patient5'}}

# Executing the tests in the above test case class
if __name__ == "__main__":
    pytest.main()
